...
void Battlefield::HandlePlayerLeaveZone(Player* plr,uint32 /*zone*/)
{
    ...
    {
        if(itr->second->m_activePlayers[plr->GetTeamId()].find(plr->GetGUID()) != itr->second->m_activePlayers[plr->GetTeamId()].end()) // If the player is in the capture point
            itr->second->HandlePlayerLeave(plr); // Remove him
    }
...
bool Battlefield::Update(uint32 diff)
{
    ...
    if(IsWarTime())
    {
        ...
        for (BfCapturePointMap::iterator itr = m_capturePoints.begin(); itr != m_capturePoints.end(); ++itr)
        {
            if (itr->second && itr->second->Update(diff))
    ...
    {
        for(uint32 i=0; i < m_GraveYardList.size() ; i++)
...
void Battlefield::InvitePlayerInZoneToQueue()
{
    for (uint8 team = 0; team < 2; ++team)
        ...
        {
            if(Player *plr = ObjectAccessor::FindPlayer((*p_itr)))
                InvitePlayerToQueue(plr);
        }
}
    ...
    if(Player* plr=sObjectAccessor->FindPlayer(guid))
        if(plr->GetZoneId() == GetZoneId())
            if (plr->GetTeam() == TEAM_ALLIANCE)
                plr->TeleportTo(KickPositionA);
            else
                plr->TeleportTo(KickPositionH);
}
...
void BfCapturePoint::HandlePlayerLeave(Player * plr)
{
    ...
    if(m_activePlayers[plr->GetTeamId()].find(plr->GetGUID()) == m_activePlayers[plr->GetTeamId()].end()) // This should not EVER happen
    {
        sLog->outError("BfCapturePoint::HandlePlayerLeave() trying to remove a player from the list when the player is not in the list (%s (GUID: %u))",plr->GetName(),plr->GetGUIDLow());
        return;
    }
        ...
        for (GuidSet::iterator itr = m_activePlayers[team].begin(); itr != m_activePlayers[team].end();++itr)
            if(Player *plr = ObjectAccessor::FindPlayer((*itr)))
                if (!m_capturePoint->IsWithinDistInMap(plr, radius) || !plr->IsOutdoorPvPActive())
                {
                    ...
                    if(m_activePlayers[team].size() < 1)
                        break;
                }
    }
