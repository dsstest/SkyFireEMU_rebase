    ...
    void ChangeControl(uint8 team,bool init/* for first call in setup*/)
    {
        switch (team)
        {
            case BATTLEFIELD_TB_TEAM_NEUTRAL:
            {
                ...
                m_TB->SendWarningToAllInZone(BATTLEFIELD_TB_TEXT_FORT_ATTACK,sObjectMgr->GetTrinityStringForDBCLocale(m_NameId),
            ...
            case BATTLEFIELD_TB_TEAM_ALLIANCE:
            {
                ...
                //Warning message
                if(!init)
                    m_TB->SendWarningToAllInZone(BATTLEFIELD_TB_TEXT_FORT_TAKEN,sObjectMgr->GetTrinityStringForDBCLocale(m_NameId),
