...
bool BattlefieldTB::SetupBattlefield()
{
    ...
    m_MapId = 732; // Map X
    m_MaxPlayer = sWorld->getIntConfig(CONFIG_TOL_BARAD_PLAYER_MAX);
    m_enable = sWorld->getBoolConfig(CONFIG_TOL_BARAD_ENABLE);
    m_MinPlayer = sWorld->getIntConfig(CONFIG_TOL_BARAD_PLAYER_MIN);
    m_MinLevel = sWorld->getIntConfig(CONFIG_TOL_BARAD_PLAYER_MIN_LVL);
...
void BattlefieldTB::OnBattleEnd(bool endbytimer)
{
    ...
    if(!endbytimer)
    {
        SendWarningToAllInZone(BATTLEFIELD_TB_TEXT_WIN_KEEP,
            ...
            ? BATTLEFIELD_TB_TEXT_ALLIANCE
            : BATTLEFIELD_TB_TEXT_HORDE));
    }
    else
    {
